...
public class ReaderCodeGen extends DelegateCodeGen implements ISchemaWalkerObserver {
	 ...
	 */
	public ReaderCodeGen(Appendable writer, IGenerateParameters dataClassParams) {
	 ...
	 */
	public ReaderCodeGen(IGenerateParameters dataClassParams, IGenerateParameters readClassParams) {
	...
	protected ISchemaWalkerObserver getWalker(LayoutDef schema) {
		if (codeGen == null) {
			codeGen = schema.getRecords().size() > 1 
					? new MultiRecordReaderCodeGen(dataClassParams, readClassParams)
					: new SingleRecordReaderCodeGen(dataClassParams, readClassParams);
		}
		return codeGen;
	}
}
